...
class BaseRegisterTest < Minitest::Test
  ...
  def part
    @part ||= Denko::SPI::BaseRegister.new(options)
  end

  def test_defaults
    new_part = Denko::SPI::BaseRegister.new bus: bus, pin: 9
  ...
  def test_options
    ...
    new_part = Denko::SPI::BaseRegister.new(options.merge(bytes: 3, pin: 10))
